...
class ChatCompletionRequest(BaseModel):
    ...
    options: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Additional options for the model (e.g., temperature, etc.)."
    )
...
async def v1_chat_completions(request: ChatCompletionRequest) -> Any:
    ...
    """
    # import ollama_inference  # Import here to ensure it's available in the Modal container
    import json
    ...
    try:
        ...
        response = ollama.chat(
            model=request.model, messages=[msg for msg in request.messages], stream=request.stream,
            format=request.format, options=request.options
        )
        return response
